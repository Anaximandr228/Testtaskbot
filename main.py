...
def callback(call):
    ...
    if call.data == 'checkdate':
        bot.send_message(call.message.chat.id, 'Введите дату для проверки') #Обработка кнопки для проверки и записи даты  Google таблицы
        bot.register_next_step_handler(call.message, check_date)


# Функция проверки правильности даты и записи в Google таблицы
def check_date(message):
    ...
    sh.sheet1.update_cell(2, 2, input_date)
